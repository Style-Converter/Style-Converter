...
internal object borderInlineEndWidth {
    fun applyBorderInlineEndWidth(prop: String, value: JsonElement, acc: BaseIR): BaseIR {
        val px = parsePx(value.jsonPrimitive.content) ?: return acc
        val current = acc.border ?: BorderIR()
        val right = current.right ?: BorderSide()
        return acc.copy(border = current.copy(right = right.copy(widthPx = px)))
    }
