...
private val INLINE_START_BORDER_STYLES = setOf(
...
private fun refactor(input: String, acc: MutableList<PropertyIR>): MutableList<PropertyIR> {
    ...
    for (t in tokens) {
        ...
        if (typeToken == null && t.lowercase() in INLINE_START_BORDER_STYLES) typeToken = t
        if (colorToken == null && parseColor(t) != null) colorToken = t
    }
	val objs = buildList<JsonObject> {
		colorToken?.let { add(buildJsonObject { put("border-inline-start-color", JsonPrimitive(it)) }) }
		typeToken?.let { add(buildJsonObject { put("border-inline-start-style", JsonPrimitive(it)) }) }
		sizeToken?.let { add(buildJsonObject { put("border-inline-start-width", JsonPrimitive(it)) }) }
	}
	for (obj in objs) parseBaseStyles(obj, acc)
	return acc
}

fun parseBorderInlineStart(styles: JsonObject, acc: MutableList<PropertyIR>): MutableList<PropertyIR> {
	val raw = styles["border-inline-start"]?.jsonPrimitive?.contentOrNull ?: return acc
