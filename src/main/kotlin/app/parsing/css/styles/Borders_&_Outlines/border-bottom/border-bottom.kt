...
private fun refactor(input: String, acc: MutableList<PropertyIR>): MutableList<PropertyIR> {
	...
	val objs = buildList<JsonObject> {
		sizeToken?.let { add(buildJsonObject { put("border-bottom-width", JsonPrimitive(it)) }) }
		borderType?.let { add(buildJsonObject { put("border-bottom-style", JsonPrimitive(it)) }) }
		colorToken?.let { add(buildJsonObject { put("border-bottom-color", JsonPrimitive(it)) }) }
	}
	for (obj in objs) parseBaseStyles(obj, acc)
	return acc
}

fun parseBorderBottom(styles: JsonObject, acc: MutableList<PropertyIR>): MutableList<PropertyIR> {
	val raw = styles["border-bottom"]?.jsonPrimitive?.contentOrNull ?: return acc
