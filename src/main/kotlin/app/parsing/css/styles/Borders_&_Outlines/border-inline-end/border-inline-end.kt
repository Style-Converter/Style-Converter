...
private val INLINE_END_BORDER_STYLES = setOf(
...
private fun refactor(input: String, acc: MutableList<PropertyIR>): MutableList<PropertyIR> {
    ...
    for (t in tokens) {
        ...
        if (typeToken == null && t.lowercase() in INLINE_END_BORDER_STYLES) typeToken = t
        if (colorToken == null && parseColor(t) != null) colorToken = t
    }
	val objs = buildList<JsonObject> {
		colorToken?.let { add(buildJsonObject { put("border-inline-end-color", JsonPrimitive(it)) }) }
		typeToken?.let { add(buildJsonObject { put("border-inline-end-style", JsonPrimitive(it)) }) }
		sizeToken?.let { add(buildJsonObject { put("border-inline-end-width", JsonPrimitive(it)) }) }
	}
	for (obj in objs) parseBaseStyles(obj, acc)
	return acc
}

fun parseBorderInlineEnd(styles: JsonObject, acc: MutableList<PropertyIR>): MutableList<PropertyIR> {
	val raw = styles["border-inline-end"]?.jsonPrimitive?.contentOrNull ?: return acc
