...
private fun refactor(input: String, acc: MutableList<PropertyIR>): MutableList<PropertyIR> {
    ...
    val objs = buildList<JsonObject> {
        sizeToken?.let { add(buildJsonObject { put("border-top-width", JsonPrimitive(it)) }) }
        borderType?.let { add(buildJsonObject { put("border-top-style", JsonPrimitive(it)) }) }
        colorToken?.let { add(buildJsonObject { put("border-top-color", JsonPrimitive(it)) }) }
    }
    for (obj in objs) parseBaseStyles(obj, acc)
    return acc
}

fun parseBorderTop(styles: JsonObject, acc: MutableList<PropertyIR>): MutableList<PropertyIR> {
	val raw = styles["border-top"]?.jsonPrimitive?.contentOrNull ?: return acc
