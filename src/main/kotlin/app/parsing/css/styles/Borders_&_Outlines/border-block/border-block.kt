...
private fun refactor(input: String, acc: MutableList<PropertyIR>): MutableList<PropertyIR> {
	...
	for (t in tokens) {
		...
		if (typeToken == null && t.lowercase() in BLOCK_BORDER_STYLES) typeToken = t
		if (colorToken == null && (t.startsWith("#") || t.lowercase().startsWith("rgb"))) colorToken = t
	}
	val objs = buildList<JsonObject> {
		colorToken?.let { add(buildJsonObject { put("border-block-color", JsonPrimitive(it)) }) }
		typeToken?.let { add(buildJsonObject { put("border-block-style", JsonPrimitive(it)) }) }
		sizeToken?.let { add(buildJsonObject { put("border-block-width", JsonPrimitive(it)) }) }
	}
	for (obj in objs) parseBaseStyles(obj, acc)
	return acc
}

fun parseBorderBlock(styles: JsonObject, acc: MutableList<PropertyIR>): MutableList<PropertyIR> {
	val raw = styles["border-block"]?.jsonPrimitive?.contentOrNull ?: return acc
