...
private fun define(input: String, acc: MutableList<PropertyIR>): MutableList<PropertyIR> {
	...
	val name = "border-bottom-left-radius"
	...
	return if (num != null) {
		...
		// preserve raw when calc() or unsupported; clear numeric/unit to let longhand win
...
fun parseBorderBottomLeftRadius(styles: JsonObject, acc: MutableList<PropertyIR>): MutableList<PropertyIR> {
	val raw = styles["border-bottom-left-radius"]?.jsonPrimitive?.contentOrNull ?: return acc
