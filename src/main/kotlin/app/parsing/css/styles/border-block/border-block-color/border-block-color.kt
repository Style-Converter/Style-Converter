...
internal object borderBlockColor {
    fun applyBorderBlockColor(prop: String, value: JsonElement, acc: BaseIR): BaseIR {
        val color = parseHexOrRgba(value.jsonPrimitive.content) ?: return acc
        val current = acc.border ?: BorderIR()
        val top = (current.top ?: BorderSide()).copy(color = color)
        val bottom = (current.bottom ?: BorderSide()).copy(color = color)
