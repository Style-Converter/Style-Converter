...
private val PRINT_COLOR_ADJUST_ALLOWED = setOf("economy","exact","auto")
...
private fun define(input: String, acc: MutableList<PropertyIR>): MutableList<PropertyIR> {
	...
	if (v !in PRINT_COLOR_ADJUST_ALLOWED) return acc
	val merged = existing.copy(value = v)
	if (idx >= 0) acc[idx] = merged else acc.add(merged)
	return acc
}

fun parsePrintColorAdjust(styles: JsonObject, acc: MutableList<PropertyIR>): MutableList<PropertyIR> {
	val raw = styles["print-color-adjust"]?.jsonPrimitive?.contentOrNull ?: return acc
